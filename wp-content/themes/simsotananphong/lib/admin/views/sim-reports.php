<div class="wrap">
	<h2>Báo cáo</h2>
	<form method="POST" enctype="multipart/form-data" id="sim-report-form" class="sim-report">
		<div class="datepicker-wrapper">
			...
			<div class="product-filter">
				...
				if (!empty($loai_sim)): ?>
					<select class="form-select" aria-label="Chọn loại thuê bao" name="loai_sim">
						<option value="" selected>- Loại thuê bao -</option>
						<?php foreach ($loai_sim as $key => $value): ?>
							<option value="<?= $key ?>" <?= isset($_GET['loai_sim']) && $_GET['loai_sim'] == $key ? 'selected' : '' ?>><?= $value['name'] ?></option>
						<?php endforeach; ?>
					</select>
				<?php endif; ?>
